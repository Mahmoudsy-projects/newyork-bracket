...
> ## 🏷️ وضعیتِ فعلی: **باگِ اصلیِ DayNet_R پیدا و فیکس شد (v1.4) — DayNet_R به ۷۴-۹۴٪ رسید، منتظرِ تأییدِ رسمی**
> اعتبارسنجیِ هم‌فیدِ XChief (دستورِ مدیرِ پروژه، هدف `DayNet_R≥۸۰٪`) نشان داد اختلافِ بروکر مقصر
> نبود — `DayNet_R` رویِ XChief هم همچنان ۴۶-۶۰٪ ماند. ریشه‌یابیِ دقیق‌تر یک باگِ واقعی پیدا کرد:
> برایِ روزهایی که نه استاپ نه TPِ کامل می‌خوردند، فرمولِ قدیمی از کلوزِ EOD استفاده می‌کرد، درحالی‌که
> `Reward_R` برگه‌ی چشمی از `MFE_Boxes/۰.۷۵` (بیشینه‌ی حرکتِ همان روز) ساخته شده بود. فیکس (v1.4)
> نرخِ تطابق را به **۷۴-۹۴٪** رساند (دو پنجره از سه‌تا بالایِ هدفِ ۸۰٪). **قدمِ بعدی:** یک اجرایِ
> تازه‌یِ MT5 با کدِ v1.4 (رویِ XChief) تا این عدد رسماً تأیید شود. بعدِ آن: دیاگنوستیکِ ۱۵روزه‌یِ
> ریورس، و منبعِ مدلِ هزینه‌یِ SpreadLog (هنوز نامعلوم — بلاک‌شده رویِ پاسخِ مدیرِ پروژه). جزئیاتِ
