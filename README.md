...
> ## 🏷️ وضعیتِ فعلی: **هدفِ ۸۰٪ی مدیرِ پروژه برآورده شد (v1.5) — DayNet_R رسمی ۸۷-۹۲٪، منتظرِ تأییدِ رسمیِ MT5**
> v1.4 یک تصحیحِ نیمه‌کاره بود: با ستونِ `Reward_R` (که معلوم شد سقفِ MFE-based است، نه نتیجه‌ی
> واقعی) سنجیده می‌شد. محمود یک ستونِ سیزدهمِ گم‌شده در فایلِ اصلی («final result at: 23:50») را
> پیدا کرد — دقیقاً نتیجه‌ی واقعیِ کلوزِ EOD، فقط برایِ روزهایِ نه‌استاپ-نه‌TP. با ground truthِ
> درست: فرمولِ اصلیِ EOD-based (v1.1-v1.3) از اول درست بوده؛ v1.5 آن را برگرداند و فرمولِ MFE-based
> را در ستونِ تشخیصیِ جداگانه‌یِ `DayNet_R_Potential` نگه داشت. نتیجه‌یِ رسمی (`compare_ground_truth.py`،
> هر سه پنجره): **۸۷.۲٪ / ۹۲.۳٪ / ۸۶.۵٪** — همه بالایِ هدفِ ۸۰٪. **قدمِ بعدی:** یک اجرایِ تازه‌یِ
> MT5 با کدِ v1.5 (رویِ XChief) تا این عدد رسماً (نه فقط شبیه‌سازیِ آفلاین) تأیید شود. بعدِ آن:
> دیاگنوستیکِ ۱۵روزه‌یِ ریورس، و منبعِ مدلِ هزینه‌یِ SpreadLog (هنوز نامعلوم). جزئیاتِ کامل:
