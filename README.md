...
> ## 🏷️ وضعیتِ فعلی: **v1.2 — دو اجرایِ واقعی انجام شد، ۳ باگ/سؤالِ باز پیدا شد، منتظرِ اجرایِ سوم**
> اجرایِ اول: سه فایلِ `NY_History_*` یک دیتا بودند (پنجره‌ها واقعاً عوض نشده بودند). اجرایِ دوم:
> هر سه پنجره درست پوشش داده شد؛ قدمِ ۲ رویِ هر سه یک باگِ pandas (`compare_ground_truth.py`،
> رشته‌ی `"None"` را NaN می‌خواند)، یک باگِ MFE-capping در `NY_DetectionLayer.mqh` (تصحیح‌شده در
> دو مرحله: v1.1 سپس v1.2 — نگاه کن به doc)، و یک **سؤالِ بازِ جدید** پیدا کرد: مهلتِ ۶۰دقیقه‌ایِ
> شکستِ معتبر برایِ پنجره‌هایِ ۰۹۰۰/۰۹۳۰ نرخِ بالایی از شکست‌هایِ واقعی-ولی-دیرتر را `None` می‌کند
> که ground truth ظاهراً حساب کرده. جزئیاتِ کامل و جدولِ نتایج:
> [`docs/NY_FrozenDefinitions.md`](docs/NY_FrozenDefinitions.md). **قدمِ بعدی:** کدِ v1.2 را دوباره
> کامپایل و هر سه پنجره را یک‌بارِ دیگر اجرا کنید؛ همزمان به «سؤالِ بازِ ۲» در doc جواب بدهید.
