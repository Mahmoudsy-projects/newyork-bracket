...
> ## 🏷️ وضعیتِ فعلی: **v1.3 — دو اجرایِ واقعی + قدمِ ۲ کامل شد، هر دو سؤالِ باز حل شد، منتظرِ اجرایِ سوم**
> اجرایِ اول: سه فایلِ `NY_History_*` یک دیتا بودند. اجرایِ دوم: هر سه پنجره درست پوشش داده شد؛
> قدمِ ۲ چند چیز پیدا کرد — یک باگِ pandas در `compare_ground_truth.py`، یک باگِ MFE-capping در
> `NY_DetectionLayer.mqh` (فیکس‌شده در v1.1→v1.2)، اختلافِ اندازه‌ی باکس (توضیح داده شد: بروکرِ
> XChief در بکتستِ چشمی در برابرِ آرینتا در اکسپورت — نه باگ)، و مهلتِ ۶۰دقیقه‌ایِ شکستِ معتبر که با
> ground truth جور درنمی‌آمد (۴۶ روزِ نامنطبق، میانگین +۰.۲۸R — سوگیریِ خوش‌بینانه). محمود تصمیم
> گرفت: به‌جایِ مهلتِ ۶۰دقیقه‌ایِ ثابت، شکست همیشه معتبر است جز ظرفِ یک‌ساعتِ پایانیِ پیش از EOD
> (v1.3 — ۴۳ از ۴۴ موردِ نامنطبق را برمی‌گرداند، طبقِ شبیه‌سازیِ آفلاین). جزئیاتِ کامل:
> [`docs/NY_FrozenDefinitions.md`](docs/NY_FrozenDefinitions.md). **قدمِ بعدی:** کدِ v1.3 را دوباره
> کامپایل و هر سه پنجره را یک‌بارِ دیگر اجرا کنید.
